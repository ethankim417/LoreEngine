...
export function IntelligenceDashboard({ articles, metrics }: IntelligenceDashboardProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("loreengine-bookmarks-updated", syncSaved);

    return () => {
      window.removeEventListener("storage", syncSaved);
      window.removeEventListener("loreengine-bookmarks-updated", syncSaved);
    };
  }, []);
  ...
  return (
    <div className="flex flex-col gap-5 pb-20 sm:gap-7 sm:pb-0 lg:gap-8">
      ...
      <div id="signals" className="scroll-mt-24 opacity-90">
...
function JumpLink({ href, label }: { href: string; label: string }) {
  return (
    <a
      href={href}
      className="inline-flex shrink-0 rounded-md px-2.5 py-2 transition hover:bg-cyan-300/10 hover:text-cyan-100"
    >
      {label}
    </a>
  );
