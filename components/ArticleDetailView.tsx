...
import { getArticleText, getCategoryLabel, getSectorLabel } from "@/lib/localizedContent";
        ...
        <article className="glass-panel overflow-hidden rounded-lg">
            ...
            <div className="relative z-10 grid gap-5 sm:min-h-[26rem] lg:grid-cols-[1fr_18rem] lg:items-end">
              <div className="max-w-3xl">
                <div className="inline-flex max-w-full flex-wrap items-center gap-2 rounded-lg border border-white/10 bg-slate-950/50 p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-xl sm:gap-3">
                  ...
                  <span className="text-sm text-slate-300">{formatDate(article.publishedAt, language)}</span>
                </div>
          ...
          <div className="grid gap-0 lg:grid-cols-[1fr_18rem]">
            <div className="space-y-8 p-5 pb-20 sm:space-y-10 sm:p-8">
              <DetailBlock title={t("whyItMatters")} body={articleText.whyItMatters} />
              <DetailBlock title={t("possibleIndustryImpact")} body={articleText.possibleImpact} />
              <DetailBlock title={t("trendAnalysis")} body={articleText.trendAnalysis} />
              <div>
                <h2 className="font-display text-xl font-bold text-white">{t("keyAffectedCompanies")}</h2>
            ...
            <aside className="border-t border-white/10 bg-black/20 p-5 sm:p-7 lg:border-l lg:border-t-0">
              ...
              <div className="mt-5 space-y-4">
                <Score label={t("industryImpact")} value={`${article.impactScore}/100`} />
                <Score label={t("momentum")} value={`+${article.trendScore}%`} />
                <Score label={t("confidence")} value={`${article.confidence}%`} />
              </div>
              <div className="mt-6">
                <p className="text-xs uppercase tracking-[0.22em] text-slate-500">{t("affectedSectors")}</p>
                <div className="mt-3 flex flex-wrap gap-2">
